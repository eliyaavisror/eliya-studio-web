...
function Content() {
  ...
  return (
    <>
      {/* HERO — full bleed, ticker meta + heavy display */}
      <section className="relative min-h-[100svh] flex flex-col justify-end pb-24 md:pb-28 pt-36 md:pt-40 text-paper">
        <div className="absolute inset-0 -z-10">
          <Image
            src="/images/hero/TO LR copy.webp"
            alt=""
            ...
            aria-hidden="true"
          />
          {/* Hero overlay — left-weighted gradient (Variation B) */}
        ...
        <div className="container-x relative">
          ...
          <div className="mt-12 md:mt-14 grid md:grid-cols-[1fr_auto] gap-8 md:gap-16 items-end">
            ...
            <div className="flex flex-wrap gap-2">
              <Link href="/visualizations" className="btn-on-dark">{t("hero.ctaPrimary")}</Link>
              <Link href="/contact" className="btn-outline-light">{t("hero.ctaSecondary")}</Link>
            </div>
          </div>

          <div className="mt-14 md:mt-16 pt-5 border-t border-paper/20 flex flex-wrap items-center justify-between gap-4 ticker text-paper/55">
            <span>120+ פרויקטים</span>
            <span className="hidden md:inline">אדריכלות / הדמיות / ייעוץ</span>
            <span>↓ גלול</span>
          </div>
        </div>
      </section>
      ...
      {/* SERVICES — Variation B: dark inverted grid */}
      <section className="section-pad bg-ink text-paper">
        <div className="container-x">
          <div className="flex flex-wrap items-end justify-between gap-6 pb-8 mb-14 md:mb-16 border-b border-paper/20">
            ...
            <p className="text-sm text-paper/60 max-w-[32ch]">שלושה תחומי ליבה תחת קורת גג אחת.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-px bg-paper/15">
            ...
            <ServiceCard num="02" title={t("services.visualizations.title")} body={t("services.visualizations.body")} tags={["חוץ","פנים","אנימציה"]}/>
            <ServiceCard num="03" title={t("services.consulting.title")} body={t("services.consulting.body")} tags={["חומרים","ליווי","קונספט"]}/>
          </div>
        </div>
      </section>
  ...
  return (
    <div className="bg-ink p-10 md:p-12 lg:p-14 min-h-[380px] flex flex-col">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {tags.map(t => (
          <span key={t} className="ticker text-[10px] text-paper/55 border border-paper/25 px-2.5 py-1.5">
            {t}
          </span>
        ))}
      </div>
